...
function PotChipView:createChipStack()
    ...
    for i, chipData in ipairs(self.potChipData_) do
        chipData:getSprite():pos(PPP.x, PPP.y + 28 + (i - 1) * GAP_WITH_CHIPS):opacity(255):addTo(self.parent_)
    end
...
function PotChipView:moveToSeat(seatId, completeCallback)
    self.moveToSeatCompleteCallback_ = completeCallback
    if self.potChipData_ then
        local chipNum = #self.potChipData_
	    local positionId = self.manager_.seatManager:getSeatPositionId(seatId)
        local position = self.manager_.seatManager:getSeatPosition(seatId)
        if self.manager_.model.isSelfDealer and self.manager_.model:isSelfDealer() then
