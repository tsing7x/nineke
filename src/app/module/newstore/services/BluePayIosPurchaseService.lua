...
function BluePayIosPurchaseService:ctor()
    ...
    if device.platform == "ios" then
        ...
        self.invokeOcMethod_("setCompleteCallback",{setupCompleteCallback = handler(self, self.onSetupComplete_),
                                        purchaseCompleteCallback = handler(self, self.onPurchaseComplete_)})
    else
        self.invokeOcMethod_ = function(method, param)
...
function BluePayIosPurchaseService:init_()
    self.active_ = true
...
function BluePayIosPurchaseService:loadGoldProductList(callback)
    self.loadGoldCallback_ = callback
    self.loadGoldRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:loadPackageProductList(callback)
    self.loadPackageCallback_ = callback
    self.loadPackageRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:loadVipProductList(callback)
    self.loadVipCallback_ = callback
    self.loadVipRequested_ = true
    self:loadProcess_()
end

function BluePayIosPurchaseService:makePurchase(pid, callback, goodsItem)
...
--下单
function BluePayIosPurchaseService:makePurchase_(pid, goodsItem)
    self.helper_:generateOrderId(pid, goodsItem.pmode, nil, function(succ, orderId, msg, data)
        if succ then
            ...
            self.logger:debug("BluePayIosPurchaseService : makePurchase smsId = " .. smsId)
            if tonumber(goodsItem.pmode) == PURCHASE_TYPE.BLUE_PAY_IOS then 
...
function BluePayIosPurchaseService:firstMakePurchase(pid, callback, goodsItem)
    self.purchaseCallback_ = callback
    self:makePurchase_(pid, goodsItem)
end
...
function BluePayIosPurchaseService:payBySMS(uid,pid,transactionId,currency,price,smsId,propsName,isShowDialog)
    ...
    self.invokeOcMethod_("payBySms", params)
end
...
function BluePayIosPurchaseService:payByBank(uid,pid,transactionId,currency,price,propsName,isShowDialog)
    ...
    self.invokeOcMethod_("payByBank", params)
end
...
function BluePayIosPurchaseService:isRequested_()
...
function BluePayIosPurchaseService:configLoadHandler_(content)
...
function BluePayIosPurchaseService:getSmsIdByPrice(category,price)
...
function BluePayIosPurchaseService:createOcMethodInvoker(className)
    if device.platform ~= "ios" then
