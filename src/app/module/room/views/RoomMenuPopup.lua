...
function RoomMenuPopup:ctor(callback)
    ...
    self.changeRoomBtn_ = cc.ui.UIPushButton.new({normal="#transparent.png", pressed="#room_pop_menu_item_pressed.png"}, {scale9=true})
        :setButtonSize(ITEM_WIDTH, ITEM_HEIGHT)
        :onButtonPressed(function() 
                self.changeRoomIcon_:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("room_pop_menu_change_pressed.png"))
            end)
        :onButtonRelease(function()
                self.changeRoomIcon_:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("room_pop_menu_change.png"))
            end)
        :onButtonClicked(function()
                nk.SoundManager:playSound(nk.SoundManager.CLICK_BUTTON)
                self:hidePanel()
    ...
    self.changeRoomIcon_ = display.newSprite("#room_pop_menu_change.png"):pos(LEFT + 48, self.changeRoomBtn_:getPositionY()):addTo(self)
