...
local HallWheelPopup = class("HallWheelPopup", function()
...
local PLAY_BTN_RES = {
    {"hall_turn_playBtn_up.png", "turn_playBtn_down.png", "hall_turn_playBtn_up.png", "hall_turn_playBtn_up.png"},
}

function HallWheelPopup:ctor(hallCtrl, viewType)
    ...
    --弹窗背景
    local backFrame = display.newSprite("#bg.png")
            :pos(POP_WIDTH*0.5, POP_HEIGHT*0.5)
            :addTo(self.mainContainer_)
    backFrame:setTouchEnabled(true)
    backFrame:setTouchSwallowEnabled(true)

    display.newSprite("#bgTitle.png")
            :pos(POP_WIDTH*0.5, POP_HEIGHT*0.5 + 236)
            :addTo(self.mainContainer_)

    display.newSprite("#bg2.png")
            :pos(POP_WIDTH*0.5, POP_HEIGHT*0.5 - 67)
            :addTo(self.mainContainer_)

    display.newSprite("#bg1.png")
            :pos(POP_WIDTH*0.5 - 143, POP_HEIGHT*0.5 - 174)
            :addTo(self.mainContainer_)

    display.newSprite("#tip.png")
            :pos(POP_WIDTH*0.5 + 222, POP_HEIGHT*0.5)
            :addTo(self.mainContainer_)

    cc.ui.UIPushButton.new({normal = "#hall_turn_btn.png", pressed = "#hall_turn_btn.png"})
        :pos(POP_WIDTH*0.5 + 225, POP_HEIGHT*0.5 - 204)
        :addTo(self.mainContainer_, 1000)
        :onButtonClicked(handler(self,function()
            device.openURL("https://play.google.com/store/apps/details?id=com.boyaa.hallgame")
            self:reportClickTimes()
        end))
...
function HallWheelPopup:faultView_()
    if not self.hallCtrl_ then
        ...
        if sz.height > POP_HEIGHT then
            bg:scale(POP_HEIGHT/sz.height)
        end
    end
end

-- 返回按钮、标题图标
function HallWheelPopup:addTopPart_()
    self.topPartNode_ = display.newNode()
        :pos(POP_WIDTH*0.5, POP_HEIGHT)
        :addTo(self.mainContainer_, 3)

    -- 返回
    local scaleVal = 1
    local BUTTON_DW, BUTTON_DH = 102,75
    px, py = -POP_WIDTH*0.5 + BUTTON_DW*1.0 - 35 + 734, -BUTTON_DH*0.8 - 14
    BubbleButton.createCommonBtn({
            iconNormalResId="#retBtn.png",
            btnNormalResId="#retBtn.png",
            btnOverResId="#retBtn.png",
...
-- 中部
function HallWheelPopup:addMiddlePart_()
    self.middlePartNode_ = display.newNode()
        :pos(POP_WIDTH*0.5 - 154, POP_HEIGHT*0.5 - 6)
    ...
    for i=1,4 do
        local sliceBg = display.newSprite("#hall_turn_slice.png")
    ...
    --扑克人
    display.newSprite("#poker.png")
        :pos(167, 142)
        :addTo(self.luckTurnNode_)

    -- 转盘边框
    self.circleSpr_ = display.newSprite("#hall_turn_circle_1.png")
        :addTo(self.luckTurnNode_)
    ...
    self.turnLightNode_ = display.newNode()
        :addTo(self.turnArenaNode_)
    self.turnLight1_ = display.newSprite("#hall_turn_light.png")
        :addTo(self.turnLightNode_)
    self.turnLight1_:setAnchorPoint(cc.p(0.5, 0.0))
    self.turnLight2_ = display.newSprite("#hall_turn_light.png")
        :addTo(self.turnLightNode_)
    ...
    display.newSprite("#hall_turn_playBg.png"):pos(0, 14):addTo(self.playNode_, 0)
    ...
    self.turnPower_ = display.newSprite("#hall_turn_power.png")
...
-- 右边转盘记录按钮
function HallWheelPopup:addRecordBtn_()
    ...
    self.recordBtnNode_:pos(POP_WIDTH*0.5-sz.width*0.5, 0)
...
function HallWheelPopup:onRecordBtnTouch_(obj, evtName)
...
function HallWheelPopup:middleAnimation_(itype)
    ...
    local animX = POP_WIDTH - RECORD_BAR_DW*0.5 - 36
    if itype == 1 then
...
-- 转盘记录面板
function HallWheelPopup:addRecordBar_()
...
function HallWheelPopup:onItemEvent_(evt)
...
function HallWheelPopup:updateTouchRect_()
...
-- 下面桌子
function HallWheelPopup:addBottomPart_()
    ...
    self.tableLeft_:pos(POP_WIDTH*0.5, sz.height*0.5)

    self.tableRight_ = display.newSprite("#turn_table.png")
        --:addTo(self.tableNode_)
    self.tableRight_:setAnchorPoint(cc.p(1, 0.5))
    self.tableRight_:pos(POP_WIDTH*0.5, sz.height*0.5)
    self.tableRight_:setScaleX(-1)

    self.tableSz_ = sz
    self.bottomPartNode_:pos(0, 0)
    ...
    self.tableEff_:pos(POP_WIDTH*0.5, sz.height*0.5*scaleVal)
    -- 剩余次数
    local px = POP_WIDTH*0.5 - 153
    local py = self.tableSz_.height*0.5+10
    self.resBg_ = display.newSprite("#bg3.png")
        :pos(px, py)
        :addTo(self.tableNode_)
    sz = self.resBg_:getContentSize()
    local msg = bm.LangUtil.getText("WHEEL", "REMAIN_COUNT2").."{0}"..bm.LangUtil.getText("WHEEL", "TIME2")
    self.feeCntLbl_ = SimpleColorLabel.html(msg, styles.FONT_COLOR.WHITE_TEXT, styles.FONT_COLOR.LIGHT_TEXT, 28, 1)
        :pos(px, py - 5)
        :addTo(self.tableNode_, 10)
end

function HallWheelPopup:getPlayTimes()
    self:setPlayTimesLoading(true)
    self.ctrl_:getPlayTimes2(function(isSucc, data, fbIntviteCount)
        if isSucc then
...
function HallWheelPopup:reportClickTimes()
    self:setPlayTimesLoading(true)
    self.ctrl_:reportClickTimes(function(isSucc, data, fbIntviteCount)
        if isSucc then
            -- 邀请好友成功赠送的次数 fbIntviteCount
            self:refreshPlayTimes_(data)
        end
...
function HallWheelPopup:refreshPlayTimes_(data,btnEnable)
    if data < 0 then
        data = 0
    end
    ...
    --self.feeCntLbl_:setPositionX(POP_WIDTH*0.5)
...
-- 转动按钮根据剩余次数改变样式
function HallWheelPopup:renderPlayBtnStyles_(value)
...
function HallWheelPopup:showCoinBuyChanceEffect_()
    local ts1 = 0.12
    local ts2 = 0.5
    local num = 10
    local list = {}
    local circleSpr
    for i=1,num do
        circleSpr = display.newSprite("#hall_turn_circle_1.png"):addTo(self.luckTurnNode_):scale(3):opacity(255*0.2)
...
function HallWheelPopup:setPlayBtnStyles(idx)
    local resId
...
-- 获取普通转盘配置信息
function HallWheelPopup:getConfig()
    self.ctrl_:getFreeConfig2(function(isSucc, data)
        if isSucc then
            self.configs_ = data
            self:renderConfig_()
        end
...
-- 呈现普通转盘配置项
function HallWheelPopup:renderConfig_()
...
-- 显示
function HallWheelPopup:onShowed()
...
function HallWheelPopup:onRemovePopup(func)
    func()
end

function HallWheelPopup:close()
    nk.PopupManager:removePopup(self)
    return self
end

-- 播放开始动画
function HallWheelPopup:playShowAnim()
    local ts = 0.25
    self.topPartNode_:moveTo(ts, POP_WIDTH*0.5, POP_HEIGHT + 0)
...
-- 播放关闭动画
function HallWheelPopup:playHideAnim(delayVal)
    local val = 0.25
    delayVal = delayVal or 0
    transition.execute(self.topPartNode_, cc.MoveTo:create(val, cc.p(POP_WIDTH*0.5, POP_HEIGHT + 80)), {delay = delayVal})
    transition.execute(self.bottomPartNode_, cc.MoveTo:create(val, cc.p(0, -self.tableSz_.height*1.0)), {delay = delayVal})
    transition.execute(self.luckTurnNode_, cc.MoveTo:create(val, cc.p(0, -POP_WIDTH*0.5)), {delay = delayVal})
end

-- 返回按钮
function HallWheelPopup:onReback()
    local val = 0
    ...
    self:performWithDelay(handler(self, self.onDelayClose_), 0.25)
...
function HallWheelPopup:onDelayClose_()
    self:close()
...
-- 添加定时器
function HallWheelPopup:addScheduler_()
...
-- 移除定时器
function HallWheelPopup:removeScheduler_()
...
-- 设置力度进度条
function HallWheelPopup:setPowerPrograss_(val)
    if val > 360 then
        val = val % 360
    end
...
-- 向PHP拉取转盘转动结果
function HallWheelPopup:onPlayFreeNow()
    self.id_ = nil
    self.ctrl_:playFreeNow2(handler(self, self.callbackPlayFreeNow_))
end

-- 转盘结果返回值
function HallWheelPopup:callbackPlayFreeNow_(isSucc, data)
    if isSucc then
...
-- 根据奖品ID获取角度
function HallWheelPopup:setDestDegreeById(id)
...
-- 判断是否有足够筹码购买转盘次数
function HallWheelPopup:isEnoughMoney()
...
function HallWheelPopup:alertGotoShop_()
    nk.ui.Dialog.new({
...
-- “开始转动” 按下与抬起状态
function HallWheelPopup:onPlayBtnHandler_(obj, evtName, ...)    
    -- 最后一次转动
    ...
    if not self.configs_ or ((not self.playTimes_ or self.playTimes_<1) and lastValue) then
        if evtName == bm.TouchHelper.TOUCH_BEGIN then
            ...
            nk.TopTipManager:showTopTip(bm.LangUtil.getText("WHEEL", "NOCHANCE"))
            return
        end
    end
    ...
    if self.ctrl_:isAllReady() then
        if self.isPlayBtnEnable_ or self.status_ == STATUS_PRESSED then
            ...
            if evtName == bm.TouchHelper.CLICK and not lastValue then
                self.isPlay_ = true
...
function HallWheelPopup:startPlay()
...
-- 转盘光效转动
function HallWheelPopup:turnLightEffect_()
...
-- 循环
function HallWheelPopup:onLoopCall_()
...
-- 转盘转动结束  谢谢惠顾不弹窗口
function HallWheelPopup:animOverCallback_()
...
function HallWheelPopup:onTurnLightCallback_()
    ...
    if self.commonAward==3 then
        ...
        HallWheelPopupGetTick = true -- 全局获取自动报名
...
-- 查找转盘配置信息
function HallWheelPopup:findItemById(id)
    return self.configs_[id + 1]
end

-- 添加监听
function HallWheelPopup:addListener()
...
-- 移除监听
function HallWheelPopup:removeListener()
...
-- 获取到转盘日志数据
function HallWheelPopup:getFreeWheelRecord()
...
-- 本地添加玩家转盘日志
function HallWheelPopup:addSelfRecordLog_(cfg)
    if not self.recordData_ then
        self.recordData_ = {}
    end
    local item = {}
...
-- 返回转盘日志数据
function HallWheelPopup:onGetFreeLkWheelRecordHandler_(evt)
...
-- 购买转盘次数回调
function HallWheelPopup:onBuyChanceHandler_(evt)
...
-- 如果日志列表数据为空，则显示提示信息
function HallWheelPopup:refreshCommitTips_()
...
-- 清理转盘日志数据
function HallWheelPopup:cleanRecordData_()
    self.recordData_ = nil
end      

function HallWheelPopup:setPlayTimesLoading(isLoading)
    if isLoading then
...
function HallWheelPopup:setRecordLoading(isLoading)
    if isLoading then
...
-- 清理
function HallWheelPopup:onCleanup()
    self:removeScheduler_()
    self:removeListener()
    HallWheelPopup.instance_ = nil

    display.removeSpriteFramesWithFile("luckturn_new_texture.plist", "luckturn_new_texture.png")
    nk.schedulerPool:delayCall(function()
        cc.Director:getInstance():getTextureCache():removeUnusedTextures()
    end, 0.1)
    HallWheelPopupGetTick =  nil
end

function HallWheelPopup.load(ctrl, view)
    if not HallWheelPopup.instance_ then
        HallWheelPopup.instance_ = true
        display.addSpriteFrames("luckturn_new_texture.plist", "luckturn_new_texture.png", function()
            HallWheelPopup.instance_ = HallWheelPopup.new(ctrl, view):show()
        end)
    end
end


return HallWheelPopup